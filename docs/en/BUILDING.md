...
This 0.13 source preview includes the pinned vendor payload, but no APK or GitHub Release. Build and sign your own APK before installation. SEC-01/02 source fixes, independent review, 45 automated tests, and build checks have passed. The build remains debuggable; full device regression testing is pending.
...
1. Complete the build below. Find the resulting 0.13 APK through `out/latest-build.json`, install it on your test phone, and launch “雷鸟随身助手” from the home screen. If signatures conflict, back up and verify the source first; do not delete existing data just to overwrite the installation. Do not substitute an old 0.12 APK.
...
Diagnostic Activities are not exported in 0.13. `lab.py run --execute` rejects that path before installing or operating on the phone. Do not re-export components to bypass this check. Install and connect through the phone app. The debug build still permits user-authorized ADB `run-as` state reads and commands for an existing session. Launching a diagnostic Activity and sending a command to an existing session are different paths.

Tests do not call cloud services or real glasses; some need the JDK/Android compilation environment. Logic tests do not replace lens confirmation. See [audit evidence](AUDIT.md) for 0.13 build status. Neither the old APK hash nor its complete device acceptance can be reused for 0.13. Full 0.13 device regression and the complete download/device workflow on a fresh Windows installation have not been revalidated.
