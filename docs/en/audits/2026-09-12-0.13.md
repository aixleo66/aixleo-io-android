...
An independent rebuild of the source snapshot also passed, using a newly generated temporary signing key and external configuration. `classes.dex`, the Manifest, and the vendor payload were byte-identical to the final local 0.13 build. Signature and alignment checks passed. The source snapshot excludes the audit-generated `out` directory, APK, configuration, and private key. See [the audited commit’s source-manifest.json](https://github.com/aixleo66/aixleo-io-android/blob/9e7e3fea481b6980a6d5e7c16544cc50985a1d8e/source-manifest.json).
