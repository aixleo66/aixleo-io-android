...
本源码预览为 0.13，包含固定厂商 payload，但不附任何 APK，也不创建 GitHub Release。先按下方“自行构建”生成自己的签名 APK，再进行安装和配置。SEC-01/02 已在本地实现修复，源码独立复核、45 项自动测试与构建检查已通过；仍为调试构建，完整真机回归尚未完成。

## 自行构建后的首次安装与配置

1. 完成下方构建后，从 `out/latest-build.json` 找到本次生成的 0.13 APK，自行安装到测试手机，再点击手机桌面的“雷鸟随身助手”启动。遇签名冲突先备份、核实来源，不要为了覆盖安装直接删除已有数据；不要另行取旧 0.12 APK 代替本次构建。
...
6. 远端知识库是可选功能，需自建兼容 `rokid-harness.v1` 的 WSS Gateway 并填写 Token；不是填任意模型 HTTP 地址就能使用。本包不含服务端或维护者的可用服务，见 [协议边界](GATEWAY.md)。不用知识库时保持「语音与默认文字提问使用知识库」关闭，使用配置的 DeepSeek。
...
0.13 的诊断 Activity 不再对外导出。`lab.py run --execute` 会在安装或操作手机前拒绝此路径；不要重新导出组件来绕过检查。请自行安装并从手机 App 启动连接。当前仍是 debug 构建，经用户授权的 ADB `run-as` 状态读取和现有会话命令工具仍可使用；启动诊断 Activity 与向已存在会话发送命令是不同路径。

这套单测不调用云或真实眼镜，部分测试需 JDK/Android 编译环境；纯逻辑通过不替代镜片确认。0.13 构建和自动验证状态以 [审核记录](../AUDIT.md) 为准，不能沿用 0.12 APK 的哈希或全部真机验收结论。当前未完成 0.13 的完整真机回归，也未宣称在全新 Windows 上重新验收全部工具下载与设备流程。
