...
按本指南构建并安装 App，再配置眼镜、语音服务和通知。仓库已包含固定版本的厂商通信依赖，暂不提供预编译 APK。构建产物保留调试能力，详情见[安全说明](SECURITY.md)。

手机 App 界面目前仍使用中文；英文文档保留实际中文按钮名并提供英文释义。文档翻译不代表 App 界面或 ASR 语言配置已完成英文化。以下命令均在仓库根目录执行，不在文档目录执行。

测试手机的眼镜连接功能要求 **Android 12+ / API 31**。Manifest 的 API 29 只是安装下限，Android 10/11 会被当前连接初始化拒绝；API 31～35 尚无完整真机验证，历史真机记录为 API 36。详见[兼容性](COMPATIBILITY.md)。

## 自行构建后的首次安装与配置

1. 完成下方构建后，从 `out/latest-build.json` 找到本次生成的 APK，自行安装到测试手机，再点击手机桌面的“Aixleo iO Android”启动。遇签名冲突先备份、核实来源，不要为了覆盖安装直接删除已有数据；不要用其他版本的安装包代替本次构建。
...
「验证阿里云 ASR（上传预置语音）」依赖原开发环境预先放入的音频，新安装没有该文件，点击会失败；请改用「选择音频上传转写」并主动选择自己有权上传的测试音频。它会上传音频并可能计费。
...
bootstrap 下载固定版本的工具链并校验哈希。已有工具可通过配置指定 java_home、build_tools、android_jar、adb。下载涉及各工具供应商的许可。
...
自动测试不调用真实云服务或眼镜，部分测试需要 JDK/Android 编译环境。完成后可按[测试指南](TESTING.md)验证实际连接和镜片显示；已记录的测试结果见[测试记录](AUDIT.md)。
