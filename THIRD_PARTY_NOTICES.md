...
本清单描述 0.13 源码预览的依赖，不是厂商授权证明。发布前须与实际源码、依赖、构建输出一致；只列名称而不保留许可证原文，不构成完整的许可履行。

特别感谢 [Turbo1123/Turbo-IO](https://github.com/Turbo1123/Turbo-IO) 的原作者与贡献者。本项目已在首页注明 iOS 项目参考关系；具体核对线索见 [来源记录](docs/PROVENANCE.md)。致谢不替代适用的版权和许可通知。
